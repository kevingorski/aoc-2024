...
async function readInput() {
  ...
  for (let rowIndex = 1; rowIndex < rows.length - 1; rowIndex++) {
    ...
    for (let colIndex = 1; colIndex < row.length - 1; colIndex++) {
      ...
      const cross = [rows[rowIndex - 1][colIndex - 1], rows[rowIndex + 1][colIndex - 1], rows[rowIndex - 1][colIndex + 1], rows[rowIndex + 1][colIndex + 1]];
      ...
      if ((topLeft === 'M' && bottomRight === 'M') || (topRight === 'M' && bottomLeft === 'M')) continue;
